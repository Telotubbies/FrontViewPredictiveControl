...
class TestCarlaManager:
    ...
    def test_camera_callback_image_conversion(self):
        ...
        # Mock camera
        self.manager.camera = Mock()
        self.manager.camera.listen = Mock()

        # Call start_camera to set up the callback
        self.manager.start_camera(callback_mock)

        # Get the camera callback function that was registered
        registered_callback = self.manager.camera.listen.call_args[0][0]

        # Call the registered callback
        registered_callback(mock_image)

        # Verify callback was called with CameraFrame
        callback_mock.assert_called_once()
        frame_arg = callback_mock.call_args[0][0]
        assert isinstance(frame_arg, CameraFrame)
        assert frame_arg.width == 640
        assert frame_arg.height == 480
        assert frame_arg.timestamp == 123456789.0
        assert frame_arg.fov == 90.0
        assert frame_arg.rgb.shape == (480, 640, 3)
    ...
    def test_camera_callback_queue_full_handling(self):
        ...
        assert self.manager.frame_queue.put_nowait.call_count == 2
    ...
    def test_camera_callback_no_callback(self):
        ...
        assert self.manager.frame_queue.put_nowait.called
    ...
    def test_rgb_conversion_bgr_to_rgb(self):
        ...
        # Verify conversion to RGB (should be red)
        frame_arg = callback_mock.call_args[0][0]
        rgb_pixel = frame_arg.rgb[0, 0]
        np.testing.assert_array_equal(rgb_pixel, [0, 0, 255])  # Red in RGB
