...
3. **Safety edge case ที่ต้องเช็คเสมอเมื่อกระทบ control/perception**:
   ...
   - Safety override (`safety/safety_override.py`) ต้องเบรกจริงเมื่อ
     obstacle ใกล้เกิน threshold
   - Stuck recovery (`safety/stuck_recovery.py`) ต้อง trigger reverse
...
5. **Smoke test แบบ headless** ถ้าทำได้ในสภาพแวดล้อม: ตรวจ import ทุก
   module ที่แก้ไม่ error, ตรวจ `config.yaml` parse ได้ปกติหลังแก้
...
- เขียน comment การทำงานของ code ตามหลักที่dev ทำกัน เป็นแบบใช้ทำอะไร (version english)
